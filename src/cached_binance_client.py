...
warnings.filterwarnings(
    action="ignore", category=Warning, message=".*websockets.*deprecated.*"
)
...
class CachedBinanceClient:
    def __init__(
        self, db_path="db/binance_data.db", api_key=None, api_secret=None, testnet=False
    ):
        """
    ...
    def _init_database(self):
        ...
        cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_symbol_interval ON price_data(symbol, interval)
        """
        )
        cursor.execute(
            """
    ...
    def get_cached_data(self, symbol, interval, start_date=None, end_date=None):
        ...
        query = """
            SELECT timestamp, open, high, low, close, volume, quote_volume,trades, taker_buy_volume, taker_buy_quote_volume
    ...
    def get_historical_klines(
        self, symbol="BTCUSDT", interval="1h", days=30, use_cache=True
    ):
        """
        ...
        print(
            f"📡 Fetching {days} days of {interval} data for {symbol} from Binance..."
        )
    ...
    def clear_cache(self, symbol=None, interval=None):
        ...
        try:
            if symbol and interval:
                cursor.execute(
                    "DELETE FROM price_data WHERE symbol = ? AND interval = ?",
                    (symbol, interval),
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch Binance price data with caching"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-cache", action="store_true", help="Disable cache (force fresh fetch)"
    )
