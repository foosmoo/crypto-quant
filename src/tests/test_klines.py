...
def test_get_historical_klines_mocked(MockClient, MockDatetime):
    ...
    MOCK_NOW = datetime(2025, 10, 28, 10, 0, 0) # Define a fixed "now" for deterministic testing (e.g., Oct 28, 2025)
    MockDatetime.now.return_value = MOCK_NOW
    MockDatetime.timedelta = timedelta # Ensure timedelta is available on the mocked datetime module
    ...
    EXPECTED_START_DATE_STRING = (MOCK_NOW - timedelta(days=TEST_DAYS)).strftime("%d %b, %Y")
