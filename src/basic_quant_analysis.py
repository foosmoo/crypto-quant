...
def detect_simple_patterns(df):
    ...
    patterns['golden_cross'] = (df['sma_20'] > df['sma_50']) & (df['sma_20'].shift(1) <= df['sma_50'].shift(1))
    patterns['death_cross'] = (df['sma_20'] < df['sma_50']) & (df['sma_20'].shift(1) >= df['sma_50'].shift(1))
    ...
    # MACD signals
    patterns['macd_bullish'] = (df['macd'] > df['macd_signal']) & (df['macd'].shift(1) <= df['macd_signal'].shift(1))
    patterns['macd_bearish'] = (df['macd'] < df['macd_signal']) & (df['macd'].shift(1) >= df['macd_signal'].shift(1))
...
def plot_analysis(df, patterns, symbol):
    ...
    for pattern_name, pattern_signal in patterns.items():
        if 'bullish' in pattern_name or 'oversold' in pattern_name or 'golden' in pattern_name:
            signal_points = df[pattern_signal]
            if len(signal_points) > 0:
                axes[0].scatter(signal_points.index, signal_points['close'], 
                              label=pattern_name, alpha=0.6, s=30)
    ...
    axes[2].plot(df.index, df['volume_sma'], label='Volume SMA', color='red', linewidth=1)
